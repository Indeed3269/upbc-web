...
foreach ($carrera in $carrerasFolders) {
    ...
    if (Test-Path $htmlFile) {
        ...
        if (-not ($content -match "section-titles\.css")) {
            $content = $content -replace '(<link rel="stylesheet" href="../../assets/css/carreras\.css">)', '$1
  <link rel="stylesheet" href="../../assets/css/section-titles.css">'
        }
        
        # Reemplazar el título "Perfil de Ingreso" por la imagen
        $content = $content -replace '<h3 class="carrera-section-title">Perfil de Ingreso</h3>', '<div class="carrera-section-title-container">
        ...
        $content = $content -replace '<h3 class="carrera-section-title">Perfil de Egreso</h3>', '<div class="carrera-section-title-container">
            <img src="' + $perfilEgresoImg + '" alt="Perfil de Egreso" class="section-title-image">
          </div>'
        
        # Reemplazar el título "Campo Laboral" por la imagen
        $content = $content -replace '<h3 class="carrera-section-title">Campo Laboral</h3>', '<div class="carrera-section-title-container">
